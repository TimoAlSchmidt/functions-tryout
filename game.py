...
sword = False
option = vraagOmIets("You're an adventurer, looking for treasure inside a dungeon. You walk into a room, the door disappearing behind you. You see 3 potions laid up on the table, a [red] one, a [blue] one and a [yellow] one. There's also a [read]able note on the table.\n", ["read", "red", "blue", "yellow"])
if option == "read":
    print("""The note says: "Two of these potions are poisonous, the third one contains the solution to leaving this room. The correct color is the same as the blood of the most common inhabitant of the forest""")
    option = vraagOmIets("Which one do you choose? [red], [blue] or [yellow]?", ["red", "blue", "yellow"])
...
if option == "red":
    ...
    print("""Yellow, like sap of trees, right? You drink the yellow potion, hoping that your answer was correct. You feel incredibly sick, but the feeling quickly subsides. The table suddenly combusts into flames, and reveals a trapdoor underneath.""")

option = vraagOmIets("You open the trapdoor, seeing a [ladder] and some water underneath. You can't gauge the depth of the water from here, but you can always try [jumping] in. You also consider [stay]ing in the room, anything better than going down there.", ["ladder", "jumping", "stay"])
...
if option != "bookshelf":
    option = vraagOmIets("In this room, you see a [sword] on a pedestal, and a [door].", ["sword", "door"])
...
if option == "door":
    ...
    sword = True
    option = vraagOmIets("You grab the sword from the pedestal, it's surprisingly light. You calmly exit the room, and enter another room. A big dining room, with tables all around. You see an skeleton with a wizard's hat, ready to cast a spell. Do you [roll] towards cover, [run] towards the skeleton or [throw] your sword.", ["roll", "run", "throw"])
...
if sword:
    option = vraagOmIets("Knocking over the table dropped some metal dining plates which you can [throw] at the skeleton, you can try [run]ning at him or you can also throw your [sword].", ["throw", "run", "sword"])
...
option = vraagOmIets("You hit the wizard skeleton straight in the face, confusing him. Now is your chance to strike. Do you run at him to [strike] him or do you [throw] your sword from a distance?", ["strike", "throw"])
...
option = vraagOmIets("You enter a treasure room of some sort and see a lot of chests around, and a door outside. Do you [investigate] the chests or do you decide to [leave] this place as fast as possible?", ["investigate", "leave"])
